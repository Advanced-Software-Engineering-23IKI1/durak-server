...
class TestDrawPile(unittest.TestCase):
    ...
    def test_004_drawpile_draw_trump(self):
        ...
        test_cards = Deck32().cards
        drawpile = DrawPile(test_cards)
        trump_suit = CardSuit.HEARTS
        trump_card = drawpile.draw_trump(trump_suit)
        self.assertIsNotNone(trump_card, "Expected trump card not found")
        self.assertEqual(trump_card.suit, trump_suit, f"Expected trump card of suit {trump_suit}, got {trump_card.suit}")
