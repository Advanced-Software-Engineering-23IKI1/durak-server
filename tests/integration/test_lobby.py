...
class TestLobby(unittest.TestCase):
    ...
    def test_002_join_game_session(self):
        ...
        while not response:
            ...
            if response:
                ...
                if not isinstance(response, LobbyStatusPackage) or (
                    len(response.players) == 1
                    and datetime.datetime.now() - loop_enter
    ...
    def test_003_lobby_updates_on_player_disconnect(self):
        ...
        event_counter = 0
        while (not end) and (
            datetime.datetime.now() - loop_enter < datetime.timedelta(seconds=10)
        ):
